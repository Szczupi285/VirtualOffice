...
{
    internal sealed class UpdateEmployeeTaskTitleHandler : IRequestHandler<UpdateEmployeeTaskTitle>
    {
        ...
        public UpdateEmployeeTaskTitleHandler(IEmployeeTaskRepository repository, IEmployeeTaskReadService readService, IMediator mediator)
        {
        ...
        public async Task Handle(UpdateEmployeeTaskTitle request, CancellationToken cancellationToken)
        {
            while (_retryCount < _maxRetryAttempts)
            {
                try
                {
                    ...
                    if (empTask._Title != request.Title)
                        empTask.SetTitle(request.Title);

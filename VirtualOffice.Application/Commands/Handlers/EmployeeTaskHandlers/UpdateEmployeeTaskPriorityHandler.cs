...
{
    internal sealed class UpdateEmployeeTaskPriorityHandler : IRequestHandler<UpdateEmployeeTaskPriority>
    {
        ...
        public UpdateEmployeeTaskPriorityHandler(IEmployeeTaskRepository repository, IEmployeeTaskReadService readService, IMediator mediator)
        {
        ...
        public async Task Handle(UpdateEmployeeTaskPriority request, CancellationToken cancellationToken)
        {
            while (_retryCount < _maxRetryAttempts)
            {
                try
                {
                    ...
                    if (empTask._Priority != request.Priority)
                        empTask.SetPriority(request.Priority);

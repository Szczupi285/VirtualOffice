...
{
    internal sealed class UpdateMeetingTitleHandler : IRequestHandler<UpdateMeetingTitle>
    {
        ...
        public UpdateMeetingTitleHandler(IMeetingRepository repository, IMeetingReadService eventReadService, IMediator mediator)
        {
        ...
        public async Task Handle(UpdateMeetingTitle request, CancellationToken cancellationToken)
        {
            while (_retryCount < _maxRetryAttempts)
            {
                try
                {
                    ...
                    if (meeting._Title != request.Title)

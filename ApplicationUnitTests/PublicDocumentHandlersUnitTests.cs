...
namespace ApplicationUnitTests
{
    public class PublicDocumentHandlersUnitTests
    {
        ...
        public async Task AddPublicDocumentAttachmentHandler_ShouldCallUpdateOnce()
        {
            ...
            _repositoryMock.Setup(r => r.GetByIdAsync(request.Id, default)).ReturnsAsync(_publicDocument);
            // Act
            await _addPubDocAttHand.Handle(request, CancellationToken.None);
            // Assert
            _repositoryMock.Verify(r => r.UpdateAsync(_publicDocument, default), Times.Once);
        }
        ...
        public async Task AddPublicDocumentHandler_ShouldCallAddOnce()
        {
            ...
            // Assert
            _repositoryMock.Verify(r => r.AddAsync(It.IsAny<PublicDocument>(), default), Times.Once);
        }
        ...
        public async Task DeletePublicDocumentHandler_ShouldCallDeleteOnce()
        {
            ...
            // Assert
            _repositoryMock.Verify(r => r.DeleteAsync(_publicDocument, default), Times.Once);
        }
